// Fig. 29.8: Options.java
// Page bean that stores user's language selection in a SessionBean.
package sessions;
...
public class Options extends AbstractPageBean
{
   ...
   public Options()
   {
      // initialize HashMap object of values to be stored in the session.
   ...
   // was selected and, if so, creates an entry for that language and
   // sets the responseText to indicate the chosen language.
   public String submitButton_action()
   {
      String message = "Welcome to Sessions! You ";
      ...
      if ( languageRadioButtonGroup.getSelected() != null )
      {
         ...
         HashMap<String, String> selections = 
            getSessionBean1().getSelectedLanguages();
         String result = selections.put( language, ISBN );        
         
         // increment numbeOfSelections in the SessionBean 
         // if user has not made this selection before
         if ( result == null )
         {
            int selected = getSessionBean1().getNumberOfSelections();
            getSessionBean1().setNumberOfSelections( ++selected );   
         } // end if
      } // end if
      else
      {
      ...
      selectionsText.setRendered( true ); // show
      selectionsValueText.setRendered( true ); // show
   ...
   public String languagesLink_action()
   {
      ...
      selectionsText.setRendered( false ); // hide
      selectionsValueText.setRendered( false ); // hide
